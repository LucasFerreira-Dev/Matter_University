...
03)	Crie uma estrutura chamada Tempo que armazene horas (inteiro), minutos (inteiro) e segundos (inteiro). Use typedef para renomeá-la para 
Horario. Escreva um programa que declare duas variáveis do tipo Horario, leia os valores do teclado e calcule a diferença em segundos entre os 
dois horários. Se o segundo horário for anterior ao primeiro, inverta-os. Imprima os dois horários no formato "HH:MM:SS" e a diferença em 
segundos
